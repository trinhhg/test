document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateLanguage(lang) {
    ...
    if (modeSelect) {
      ...
      console.error('Không tìm thấy phần tử chọn mode select');
    }
  }

  function updateModeButtons() {
    const renameMode = document.getElementById('rename');
    const deleteMode = document.getElementById('delete');
  ...
  function updateButtonStates() {
    ...
    if (matchCaseButton) {
      ...
      matchCaseButton.style.background = matchCaseEnabled ? '#28a' : '#6c757d';
    } else {
  ...
  function loadModes() {
    ...
    let settings = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || { modes: [], matchCase: false } };
        ...
        if (currentMode !== 'default') {
          if (confirm(`Bạn có chắc chắn muốn xóa chế độ "${currentMode}"?`)) {
            ...
            if (settings.modes[currentMode]) {
              delete settings.modes[currentMode].currentMode);
              localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(settings));
              );
              currentMode = 'default';
              loadModes();
              showNotification(translations[currentLang].modeDeleted.replace('{mode}', currentMode));
            }
          });
          }
        });
      ...
      if (buttons.addModeButton) {
        buttons.addModeButton.addEventListener('click', () => {
          ...
          if (newMode && !newMode.includes('mode_') && newMode.trim() !== '' && newMode !== 'default') {
            ...
            currentMode = newMode;
            loadModes();
            showNotification(translations[currentLang].modeCreated.replace('{mode}', mode));
            } else {
              showNotification(translations[currentLang].invalidModeName, 'error');
            }
          });
        ...
        if (buttons.copyModeButton) {
          buttons.copyModeButton.addEventListener('click', () => {
            ...
            const newMode = prompt(translations[currentLang].newModePrompt);
            if (newMode && !newMode.includes('mode_' && mode) && newMode.trim() !== '' && newMode !== 'default') {
              let settings = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || { modes: { default: { pairs: [], matchCase: false } } };
              if (settings.modes[newMode]) {
                showNotification(translations[currentLang].invalidModeName, 'error');
                return;
        ...
        if (buttons.modeSelect) {
          buttons.modeSelect.addEventListener('change', (e) => {
            ...
            loadSettings();
            showNotification(translations[currentLang].switchedMode.replace('{mode}', currentMode));
            updateModeButtons();
          });
        } else {
